...
def multi_sampler(cohorts,samples=1,lump_emails=set()):
    
    for cohort in cohorts:
        print(cohort)
        emails = list(set(ids_to_emails(cohort+'.csv')) - lump_emails)
        new_df,name = sampler(emails,samples,cohort)
